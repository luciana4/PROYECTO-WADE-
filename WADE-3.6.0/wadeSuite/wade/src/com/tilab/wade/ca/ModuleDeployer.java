...
public class ModuleDeployer extends AchieveREInitiator {

	public static final long DEFAULT_DEPLOY_TIMEOUT = 60000;
	...
	private long timeout = DEFAULT_DEPLOY_TIMEOUT;
	private Deploy deploy;
	private AID caAID;

	public ModuleDeployer(byte[] moduleContent, String moduleName, boolean rebuildClassLoader) {
		super(null, null);
		
		init(moduleContent, moduleName, rebuildClassLoader);
	}

	public ModuleDeployer(String jarFileName, boolean rebuildClassLoader) throws IOException {
		super(null, null);
		...
		if(jarFileName != null) {
			...
			moduleContent = FileUtils.file2byte(jarFile);
			moduleName = jarFile.getName().replaceFirst("[.][^.]+$", "");
		}
		
		init(moduleContent, moduleName, rebuildClassLoader);
	}

	private void init(byte[] moduleContent, String moduleName, boolean rebuildClassLoader) {
		deploy = new Deploy();
		deploy.setModuleName(moduleName);
		deploy.setModuleContent(moduleContent);
		deploy.setRebuildClassLoader(rebuildClassLoader);
	}
	...
	protected Vector prepareRequests(ACLMessage msg) {
		...
		try {
			...
			myAgent.getContentManager().fillContent(request, new Action(caAID, deploy));
