...
public class ModuleUndeployer extends AchieveREInitiator {
	...
	public ModuleUndeployer(String moduleName, boolean rebuildClassLoader) {
		super(null, null);
		
		undeploy = new Undeploy();
		undeploy.setModuleName(moduleName);
		undeploy.setRebuildClassLoader(rebuildClassLoader);
	}
	...
	protected Vector prepareRequests(ACLMessage msg) {
		...
		try {
			...
			myAgent.getContentManager().fillContent(request, new Action(caAID, undeploy));
